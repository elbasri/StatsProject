...
function Results() {
  ...
  useEffect(() => {
    const fetchResultData = async () => {
      try {
        const response = await axios.get(`http://https://statsprojectapi.maktab.ma/api/results/${resultId}/`);
        ...
        <MDTypography variant="h4" fontWeight="medium" mt={1}>
            <div>
              ...
              {resultData.graph !== 'noGraph' ? (
                ...
                src={`http://https://statsprojectapi.maktab.ma/${resultData.graph}`} // Update the URL as needed
                title="Graph"
